...
{
    /**
     * Para mostrar la vista de formulario de denuncias
     */
    public function create()
    {
        // // Temporal ---------------------------------------
        //     // ya que no existe una tabla para las opciones aún:
        //     
        // // Temporal ---------------------------------------

        $opcion_custom_form = null; 

        $setting = TenantSetting::where('name', 'Form')->first();

        if(isset($setting) && $setting->value == 1) {
            $opcion_custom_form = true;
        } else {
            $opcion_custom_form = false;
        }

        // Carga todas las preguntas para mostrarlas en pantalla
        $questions = Question::all();

        return view('denuncia.create', compact('questions', 'opcion_custom_form'));
    }

    /**
     * Para guardar las nuevas denuncias
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Para guardar las respuestas de la denuncia
            $respuestas = $request->input('respuestas');
            // $preguntas_con_respuestas = [];

            foreach ($respuestas as $question_id => $answer) {
                // $preguntas_con_respuestas[] = [
                ...
                FormAnswer::create([
                    'ans_text' => is_array($answer) ? implode(', ', $answer) : $answer,
                    'question_id' => $question_id,
                    'denuncia_id' => $denuncia->id
                ]);
            }

            // dd($preguntas_con_respuestas);
            ...
            Log::error('Existe un error al guardar la denuncia: ' . $e);
            DB::rollBack();
        }
    }
